...
export function AboutPage() {
  ...
  const i = locale !== "en" ? ABOUT_I18N[locale as keyof typeof ABOUT_I18N] : null;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        ...
        <p className="text-sm text-foreground/80 leading-relaxed">
          {i?.intro2 || <>This app offers two ways to pray the Liturgy of the Hours: the <strong>Original
      ...
      <div className="bg-card rounded-xl border border-border p-4 space-y-2">
        <h3 className="font-semibold text-foreground">{i?.paterTitle || "The Original Pater Count"}</h3>
        <p className="text-sm text-foreground/80 leading-relaxed">
          {i?.paterDesc || "Chapter III of the Rule of St. Francis (1223) prescribes that lay brothers who cannot read the Psalter should instead pray Our Fathers at each canonical hour. The exact distribution is:"}
        </p>
        <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-sm">
          ...
          <span className="text-muted-foreground font-semibold border-t border-border pt-1">
            {locale === "es" ? "Total Diario" : locale === "it" ? "Totale Giornaliero" : locale === "fr" ? "Total Quotidien" : locale === "zh" ? "每日总计" : "Daily Total"}
          </span>
      ...
      {/* Prayer Modes */}
      <div className="bg-card rounded-xl border border-border p-4 space-y-2">
        <h3 className="font-semibold text-foreground">{i?.howTitle || "How to Use This App"}</h3>
        <div className="space-y-3 text-sm text-foreground/80 leading-relaxed">
          <div>
            <p className="font-medium text-foreground">{i?.howFullDay || "Full Day (recommended)"}</p>
            <p>{i?.howFullDayDesc || "Pray each Hour at its canonical time throughout the day. The app suggests the next Hour based on the time of day and highlights it on the home screen."}</p>
          </div>
          <div>
            <p className="font-medium text-foreground">{i?.howBusy || "Busy Parent Mode"}</p>
            <p>{i?.howBusyDesc || "Focus on Lauds (morning), Vespers (evening), and Compline (bedtime). These three are the most important Hours and take only a few minutes each."}</p>
          </div>
          <div>
            <p className="font-medium text-foreground">{i?.howMissed || "If You Miss an Hour"}</p>
            <p>{i?.howMissedDesc || <>Simply continue with the next one. &ldquo;If you missed Terce, continue with Sext.&rdquo; There is no guilt in Franciscan prayer — only grace and joy in returning.</>}</p>
          </div>
        </div>
      </div>
      ...
      {/* Franciscan Crown */}
      <div className="bg-card rounded-xl border border-border p-4 space-y-2">
        <h3 className="font-semibold text-foreground">{i?.crownTitle || "The Franciscan Crown Rosary"}</h3>
        <p className="text-sm text-foreground/80 leading-relaxed">
          {i?.crownDesc || <>Also called the Seraphic Rosary, the Crown consists of 7 decades (not 5)
      ...
      {/* Stations */}
      <div className="bg-card rounded-xl border border-border p-4 space-y-2">
        <h3 className="font-semibold text-foreground">{i?.stationsTitle || "The Stations of the Cross"}</h3>
        <p className="text-sm text-foreground/80 leading-relaxed">
          {i?.stationsDesc || "The Stations are a Franciscan gift to the Church. As custodians of the Holy Land since 1217, the Franciscans created this devotion so that those who could not make a pilgrimage to Jerusalem could spiritually walk the Via Dolorosa. The 14 stations were formally established by Pope Clement XII in 1731 and entrusted to the Franciscan Order."}
        </p>
      </div>

      {/* Content Sources */}
      <div className="bg-card rounded-xl border border-border p-4 space-y-2">
        <h3 className="font-semibold text-foreground">{i?.sourcesTitle || "Sources & Credits"}</h3>
      ...
      <div className="text-center py-4">
        <p className="text-lg font-semibold text-franciscan italic">
          {i?.paxFooter || "Pax et Bonum"}
        </p>
        <p className="text-xs text-muted-foreground mt-2">
          {i?.builtWith || "Built with love for the greater glory of God"}
        </p>
